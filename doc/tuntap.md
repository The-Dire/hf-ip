...
- 使用select()同时控制这两个描述符;如果数据来自任何一个描述符，它将被写入另一个描述符。

- 由于程序使用TCP，接收方将看到单一的数据流，这使得识别数据包边界变得困难。因此，当一个包或帧被写入网络时，它的长度被预留一些作为标志位(2字节)然后才是实际的包的数据。

- 当数据从tap_fd描述符进入时，一次读取一个完整的数据包或帧;因此，可以直接将其写入网络，并预先写明其长度。由于这个长度数是一个短整型数，因此长于一个字节，以“原始”二进制格式写入，ntohs()/htons()用于在具有不同字节序(大小端)的机器之间进行互操作。

- 当数据从网络传入时，由于前面提到的技巧，可以通过读取流中先于它的两个字节的长度来知道下一个包将会有多长。当读取包时，我们将它写入到tun/tap接口描述符中，内核将在那里接收到它，认为它来自“连线”。

源代码地址: [simple_tunnel源代码](https://github.com/The-Dire/hf-ip/blob/master/doc/simpletunnel/simpletun.c)