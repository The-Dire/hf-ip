...
**注:tun和tap是有区别的。** 区别为:用户层程序通过tun设备只能读写IP数据包，而通过tap设备能读写链路层数据包，类似于普通socket和raw socket的差别一样，处理数据包的格式不一样。

#### 一个简单的例子

这里写了一个程序，它收到tun设备的数据包之后，只打印出收到了多少字节的数据包，其它的什么都不做。有关tun/tap的代码细节请参考 [tun/tap详解1](https://github.com/The-Dire/hf-ip/blob/master/doc/tuntap.md)
...
he@ubuntu:~$ sudo ip addr add 192.168.3.11/24 dev tun1

#默认情况下，tun1没有起来，用下面的命令将tun1启动起来
he@ubuntu:~$ sudo ip link set tun1 up
...
he@ubuntu:~$ ping -c 4 192.168.3.12
...
tun/tap作为结构简单的虚拟网络设备能够理解很多,这也能够为更复杂的虚拟网络设备(比如网桥做铺垫)。