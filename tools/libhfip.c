...
static int hfip_socks_count = 0;
static LIST_HEAD(hfip_socks);

static inline struct hfip_sock *hfip_get_sock(int fd) {
    struct list_head *item;
    struct hfip_sock *sock;

    list_for_each(item, &hfip_socks) {
        sock = list_entry(item, struct hfip_sock, list);
...
static int free_socket(int hffd)
{
    return _close(hffd);
}

static int transmit_hfip(int hffd, struct ipc_msg *msg, int msglen)
{
    char *buf[RCBUF_LEN];

    // Send mocked syscall to hf-ip
    if (_write(hffd, (char *)msg, msglen) == -1) {
        perror("Error on writing IPC");
    }

    // Read return value from hf-ip
    if (_read(hffd, buf, RCBUF_LEN) == -1) {
...
int socket(int domain, int type, int protocol)
{
    ...
    int hffd = init_socket("/tmp/hfip.socket");

    sock = hfip_alloc();
    sock->hffd = hffd;
    list_add_tail(&sock->list, &hfip_socks);
    hfip_socks_count++;
    ...
    int sockfd = transmit_hfip(sock->hffd, msg, msglen);
    ...
    hf_sock_dbg("Socket called", sock);
...
int close(int fd)
{
    struct hfip_sock *sock = hfip_get_sock(fd);

    if (sock == NULL) {
        /* No hf-ip IPC socket associated */
        return _close(fd);
    }

    hf_sock_dbg("Close called", sock);
    ...
    rc = transmit_hfip(sock->hffd, msg, msglen);
    free_socket(sock->hffd);
...
{
    struct hfip_sock *sock = hfip_get_sock(sockfd);
    ...
    hf_sock_dbg("Connect called", sock);
    ...
    return transmit_hfip(sock->hffd, msg, msglen);
}
...
ssize_t read(int sockfd, void *buf, size_t len)
{
    ...
    // Send mocked syscall to hf-ip
    if (_write(sock->hffd, (char *)msg, msglen) == -1) {
    ...
    // Read return value from hf-ip
    if (_read(sock->hffd, rbuf, rlen) == -1) {
...
ssize_t sendto(int fd, const void *buf, size_t len,
               ...
               socklen_t dest_len)
{
    if (!hfip_get_sock(fd)) return _sendto(fd, buf, len,
...
{
    if (!hfip_get_sock(fd)) return _recvfrom(fd, buf, len,
...
int poll(struct pollfd *fds, nfds_t nfds, int timeout)
{
    ...
    struct pollfd *hfip_fds[nfds];
    int hfip_nfds = 0;
    int kernel_nfds = 0;
    int hfip_sock = 0;

    struct hfip_sock *sock = NULL;

    for (int i = 0; i < nfds; i++) {
        struct pollfd *pfd = &fds[i];
        if ((sock = hfip_get_sock(pfd->fd)) != NULL) {
            hfip_fds[hfip_nfds++] = pfd;
            hfip_sock = sock->hffd;
        } else {
    ...
    int blocking = 0;
    if (kernel_nfds > 0 && hfip_nfds > 0 && timeout == -1) {
        /* Cannot sleep indefinitely when we demux poll 
           with both kernel and hfip fds */
        timeout = 100;
        blocking = 1;
    }

    hf_dbg("Poll called with kernel_nfds %d hfip_nfds %d timeout %d", kernel_nfds, hfip_nfds, timeout);

    for (;;) {
        int events = 0;
        if (kernel_nfds > 0) {
            for (int i = 0; i < kernel_nfds; i++) {
                hf_dbg("Kernel nfd %d events %d timeout %d", kernel_fds[i]->fd, kernel_fds[i]->events, timeout);
            }
        ...
        int msglen = sizeof(struct ipc_msg) + sizeof(struct ipc_poll) + pollfd_size * hfip_nfds;
        struct ipc_msg *msg = alloca(msglen);
        ...
        for (int i = 0; i < hfip_nfds; i++) {
            pfd = &data->fds[i];
            pfd->fd = hfip_fds[i]->fd;
            pfd->events = hfip_fds[i]->events;
            pfd->revents = hfip_fds[i]->revents;
        }

        if (_write(hfip_sock, (char *)msg, msglen) == -1) {
            perror("Error on writing IPC poll");
            errno = EAGAIN;
            return -1;
        }

        int rlen = sizeof(struct ipc_msg) + sizeof(struct ipc_err) + pollfd_size * hfip_nfds;
        char rbuf[rlen];
        memset(rbuf, 0, rlen);

        // Read return value from hf-ip
        if (_read(hfip_sock, rbuf, rlen) == -1) {
        ...
        for (int i = 0; i < hfip_nfds; i++) {
            hfip_fds[i]->events = returned[i].events;
            hfip_fds[i]->revents = returned[i].revents;
        }
        ...
        if (result > 0 || !blocking) {
            for (int i = 0; i < nfds; i++) {
                hf_dbg("Returning counts %d nfd %d with revents %d events %d timeout %d", result, i, fds[i].revents, fds[i].events, timeout);
            }
               ...
               const void *optval, socklen_t optlen)
{
    ...
    hf_sock_dbg("Setsockopt called", sock);
               ...
               void *optval, socklen_t *optlen)
{
    ...
    hf_sock_dbg("Getsockopt called: level %d optname %d optval %d socklen %d",
    ...
    hf_sock_dbg("Got getsockopt level %d optname %d optval %d socklen %d",
                 sock, optres->level, optres->optname, *(int *)optres->optval, optres->optlen);

    int val = *(int *)optres->optval;

    /* hf-ip probably encoded the error value as negative */
    val *= -1;
...
{
    struct hfip_sock *sock = hfip_get_sock(socket);
    if (sock == NULL) return _getpeername(socket, address, address_len);

    hf_sock_dbg("Getpeername called", sock);
    ...
    hf_sock_dbg("Got getpeername fd %d addrlen %d sa_data %p",
                ...
                socklen_t *restrict address_len)
{
    ...
    hf_sock_dbg("Getsockname called", sock);
    ...
    hf_sock_dbg("Got getsockname fd %d addrlen %d sa_data %p",
                 sock, nameres->socket, nameres->address_len, nameres->sa_data);
...
int fcntl(int fildes, int cmd, ...)
{
    ...
    struct hfip_sock *sock = hfip_get_sock(fildes);
    ...
    switch (cmd) {
    case F_GETFL:
        hf_sock_dbg("Fcntl GETFL", sock);

        rc = transmit_hfip(sock->hffd, msg, msglen);
        break;
    case F_SETFL:
        hf_sock_dbg("Fcntl SETFL", sock);
