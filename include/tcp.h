...
// tcp选项数据结构
struct tcp_options {
    uint16_t options;// 什么类型的选项
    uint16_t mss; // 最大段大小
    uint8_t sack; // 选择性确认
};
// MSS从对端接收到的最大的报文长度的结构,当你选择了tcp选项的MSS字段才会有此结构体
struct tcp_opt_mss {
    uint8_t kind;
    uint8_t len;
    uint16_t mss;
...
// tcp十一种状态
enum tcp_states {
    TCP_LISTEN, /* TCP正等待从对端TCP结点发来的连接请求 */
    TCP_SYN_SENT, /* TCP发送了一个SYN报文,代表应用程序
                    执行了一个主动打开的操作,并等待对端回应以此完成连接建立 */
    TCP_SYN_RECEIVED, /* 此前处于LISTEN状态的TCP结点接收到了对端发送的SYN报文,并
                        已经通过发送SYN/ACK报文做出了响应 */
    TCP_ESTABLISHED, /* 与对端TCP结点间的连接建立完成 */
    TCP_FIN_WAIT_1, /* 应用程序关闭了连接 */
    TCP_FIN_WAIT_2, /* 之前FIN_WAIT_1状态的TCP结点现在已经收到了
                        对端TCP结点发来的ACK */
    TCP_CLOSE, /* 关闭状态 */
    TCP_CLOSE_WAIT, /* TCP结点从对端收到FIN报文后处于该状态,
                    该状态以及接下来的一个状态都同应用程序执行的被动关闭有关 */
    TCP_CLOSING, /* 之前处于FIN_WAIT_1状态的结点正在等待对端发送ACK,
                    但却收到了FIN */
    TCP_LAST_ACK, /* 应用程序执行被动关闭 */
    TCP_TIME_WAIT, /* 完成主动关闭后,TCP结点接收到了FIN报文 */
};
